...
from db.enums import DocumentSearchMode, DocumentEmbeddingDistanceMetric
...
class CreateContextRequest(BaseModel):
    ...
    search_mode: DocumentSearchMode = Field(DocumentSearchMode.SEARCH_WITH_CITATIONS, title="Search mode", description="Search mode of the context")
    retrieval_length: int = Field(1024, title="Retrieval length", description="Retrieval length of the context")
    docs_to_retrieve: int = Field(10, title="Docs to retrieve", description="Number of documents to retrieve")
    max_doc_length: int = Field(256, title="Max doc length", description="Maximum document length")
    doc_overlap_length: int = Field(64, title="Doc overlap length", description="Document overlap length")
    embedding_model: EmbeddingModel = Field(EmbeddingModel.BAAI_BGE_BASE_EN, title="Embedding model", description="Embedding model of the context")
    embedding_dimension: int = Field(768, title="Embedding dimension", description="Embedding dimension of the context")
    distance_metric: DocumentEmbeddingDistanceMetric = Field(DocumentEmbeddingDistanceMetric.COSINE, title="Distance metric", description="Distance metric of the context")
