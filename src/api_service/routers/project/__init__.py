...
from .types import CreateProjectRequest, GetAllProjectsRequest
...
@router.post("/project/getall")
async def get_all_projects(request: Request, data: GetAllProjectsRequest):
    if not data.owner_id:
        raise HTTPException(status_code=400, detail="owner_id is not in the request.")

    async with db.session() as session:
        get_project_query = select(Project).where(Project.owner_id == data.owner_id).limit(data.limit).offset(data.offset)
