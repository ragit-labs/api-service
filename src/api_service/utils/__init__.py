...
from db.models import ContextFile
...
def create_text_embeddings(documents: List[str], model_name: str = EmbeddingModel.BAAI_BGE_BASE_EN, max_length: int = 512):
...
async def update_status(context_id: str, file_id: str, status: EmbeddingStatus):
    async with db.session() as session:
        context_file_query = select(ContextFile).where(ContextFile.context_id == context_id, ContextFile.file_id == file_id)
...
def partition_and_insert(context_id: str, file_id: str, file_key: str, batch_size: int):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Could not download file from S3. {str(ex)}")
